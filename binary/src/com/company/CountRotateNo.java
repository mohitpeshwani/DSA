...
public class CountRotateNo {
    public static void main(String[] args) {
        int[] arr = {6, 7, 8, 0, 1, 2, 3, 4, 5};
        System.out.println(findnoofrotation(arr));
    }
    static int findnoofrotation(int[] num){
        int pivot=findPivotWithDuplicates(num);
        return pivot+1;
    }
    ...
    static int findPivotWithDuplicates (int []array){
        ...
        while (s <= e) {
            ...
            else if(array[s] < array[mid] || (array[s] == array[mid] && array[mid] > array[e])) {
                s = mid + 1;
            } else {
                e = mid - 1;
            }
